...
/* ---------- river5 v13c (direct vtable, hybrid fixed+content-dep shuffle) ---------- */

static void river5_v13c_one(const void *in, size_t len, uint8_t *out)
{
    RIVER5_VTABLE_AESNI_V13C.one_shot(in, len, NULL, out);
}
static void *river5_v13c_new(void)
{
    return RIVER5_VTABLE_AESNI_V13C.new_state(NULL);
}
static void  river5_v13c_update(void *s, const void *d, size_t n)
{
    RIVER5_VTABLE_AESNI_V13C.update((river5_ctx_t *)s, d, n);
}
static void  river5_v13c_digest(void *s, uint8_t *out)
{
    RIVER5_VTABLE_AESNI_V13C.finalize((river5_ctx_t *)s, out);
}
static void  river5_v13c_free(void *s)
{
    RIVER5_VTABLE_AESNI_V13C.free_state((river5_ctx_t *)s);
}

static const hash_impl river5_v13c_impl = {
    .name        = "river5-v13c",
    .output_bits = 128,
    .one_shot    = river5_v13c_one,
    .new_state   = river5_v13c_new,
    .update      = river5_v13c_update,
    .digest      = river5_v13c_digest,
    .free_state  = river5_v13c_free,
};
