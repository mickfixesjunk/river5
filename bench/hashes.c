...
/* ---------- river5 v8 (direct vtable, deeper-finalize experiment) ---------- */

static void river5_v8_one(const void *in, size_t len, uint8_t *out)
{
    RIVER5_VTABLE_AESNI_V8.one_shot(in, len, NULL, out);
}
static void *river5_v8_new(void)
{
    return RIVER5_VTABLE_AESNI_V8.new_state(NULL);
}
static void  river5_v8_update(void *s, const void *d, size_t n)
{
    RIVER5_VTABLE_AESNI_V8.update((river5_ctx_t *)s, d, n);
}
static void  river5_v8_digest(void *s, uint8_t *out)
{
    RIVER5_VTABLE_AESNI_V8.finalize((river5_ctx_t *)s, out);
}
static void  river5_v8_free(void *s)
{
    RIVER5_VTABLE_AESNI_V8.free_state((river5_ctx_t *)s);
}

static const hash_impl river5_v8_impl = {
    .name        = "river5-v8",
    .output_bits = 128,
    .one_shot    = river5_v8_one,
    .new_state   = river5_v8_new,
    .update      = river5_v8_update,
    .digest      = river5_v8_digest,
    .free_state  = river5_v8_free,
};
